                ...
                <div key={item.menu_item_id} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.9rem 0', borderBottom: '1px solid #EDE5D0' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: '0.3rem' }}>
                      <button onClick={() => removeItem(item.menu_item_id)} style={{ width: 24, height: 24, border: '1px solid #EDE5D0', borderRadius: '50%', background: 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Minus size={10} /></button>
                      <span style={{ fontWeight: 600, fontSize: '0.9rem', minWidth: 20, textAlign: 'center' }}>{item.quantity}</span>
                      <button onClick={() => addItem(item)} style={{ width: 24, height: 24, border: '1px solid #EDE5D0', borderRadius: '50%', background: 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Plus size={10} /></button>
                    </div>
                  </div>
                </div>
              ))}
        ...
        {items.length > 0 && (
          <div style={{ padding: '1.25rem 1.75rem', borderTop: '1px solid #EDE5D0', background: '#F5F0E8' }}>
            ...
            <div style={{ marginBottom: '1rem' }}>
              {[['Subtotal', formatPrice(subtotal), ''], discount > 0 ? [`Discount (${couponCode})`, `-${formatPrice(discount)}`, 'green'] : null, ['GST (5%)', formatPrice(tax), ''], ['Total', formatPrice(total), 'bold']].filter(Boolean).map((row) => {
                const [label, value, extra] = row as string[]
                return (
                  <div key={label} style={{ display: 'flex', justifyContent: 'space-between', padding: '0.3rem 0', fontSize: extra === 'bold' ? '1rem' : '0.88rem', fontWeight: extra === 'bold' ? 700 : 400, color: extra === 'green' ? '#2E7D32' : '#2C1A0E', borderTop: extra === 'bold' ? '1px solid #EDE5D0' : 'none', marginTop: extra === 'bold' ? '0.5rem' : 0, paddingTop: extra === 'bold' ? '0.75rem' : '0.3rem' }}>
                    <span style={{ color: extra === 'bold' ? '#2C1A0E' : '#7A6245' }}>{label}</span>
                    <span>{value}</span>
                  </div>
                )
              })}
